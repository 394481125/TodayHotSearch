...
### 1. 多平台实时热点聚合
系统支持一键轻松浏览包括以下 **9大主流平台** 的实时热搜及热议话题：
...
*   **直接抓取引擎（Direct Engines）**：系统率先使用经过优化的、基于 OkHttp 的轻量化网络解析器。模拟真实浏览器请求头（`User-Agent`/`Referer`），直接请求各大官网公开 API 接口进行毫秒级原生解析（如 `weibo.com`、`zhihu.com`等），确保获取最底层、无延迟、未受缓存污染的一手原始热度数据。
*   **第三方 Fallback 降级通道**：当目标官方服务器限制严格、网络超时或证书失效时，自动无缝切换至韩小韩备用 API 接口（`https://api.vvhan.com/api/hotlist`），作为最高保障，确保用户在任何环境下都能顺畅刷新榜单。

### 3. 本地轻量离线缓存系统 (SQLite Room Database)
...
```
*   `type` 可选值：`wb`、`zhihu`、`baidu`、`bilibili`、`douyin`、`toutiao`、`it`、`36kr`、`daily` 等。
